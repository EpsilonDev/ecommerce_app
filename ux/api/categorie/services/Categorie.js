...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Categorie.associations
  ...
  fetch: (params) => {
    ...
    return Categorie
      .findOne(_.pick(params, _.keys(Categorie.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Categorie.create(data);

    // Create relational data and return the entry.
    return Categorie.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Categorie.associations.map(a => a.alias));
    const data = _.omit(values, Categorie.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Categorie.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Categorie.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Categorie.attributes).reduce((acc, curr) => {
      switch (Categorie.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
